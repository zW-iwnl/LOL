...
function TestCaseDetail() {
  ...
  return <div className="workspace-page">
    <Link className="text-sm text-cyan-700" to={safeReturn(params.get("returnTo"), "/test-cases")}>{params.has("returnTo") ? "← Zpět na předchozí práci" : "← Zpět do Repository"}</Link>
    <header className="flex flex-wrap justify-between gap-3"><div><h2 className="text-2xl font-semibold">{caseItem.code} · {caseItem.title}</h2><p className="text-sm text-slate-500">{archived ? "Archivovaný scénář" : caseItem.current_approved_version_id ? `Publikovaná verze ${caseItem.version}` : "Dosud nemá schválenou verzi"}</p></div><Link className="text-cyan-700" to={withReturn(`/test-case-approvals?q=${encodeURIComponent(caseItem.code)}`, location.pathname + location.search)}>Schvalování tohoto scénáře</Link></header>
