...
export function ExecutionSteps({ steps, results, users, disabled, savingStepId, onResult }: Props) {
  ...
  return <section aria-label="Kroky testu">
    <div className="flex items-center justify-between gap-2 border-y border-slate-200 bg-slate-50 px-3 py-2">
      <h3 className="text-sm font-semibold">Kroky testu</h3><span className="text-xs text-slate-500">{completed}/{tests.length} vyhodnoceno · kroky se ukládají ihned</span>
    </div>
    {tests.length === 0 && <p className="bg-sky-50 p-3 text-sm text-sky-800">{steps.length ? "Test obsahuje pouze informační kroky." : "Test nemá definované kroky."} Celkový výsledek lze uložit ručně.</p>}
    ...
    {steps.map(step => {
      ...
      return <div id={`step-${step.id}`} key={step.id} className={`execution-step-columns execution-step ${information ? "bg-sky-50/60" : ""}`}>
        <span className="text-xs font-semibold text-slate-500">{step.step_order}</span>
        <div className={information ? "execution-information" : ""}>
          {information && <span className="mr-2 text-xs font-medium text-sky-800">Informace</span>}
          <p className={`whitespace-pre-wrap break-words text-sm ${information ? "inline" : ""}`}>{step.action}</p>
          {step.test_data && <p className="mt-1 whitespace-pre-wrap break-words text-xs text-slate-600"><b>Data:</b> {step.test_data}</p>}
          {information && step.note && <p className="mt-1 whitespace-pre-wrap text-xs text-slate-600">{step.note}</p>}
        </div>
        {!information && <>
          <div><span className="execution-mobile-label">Očekávaný výsledek</span><p className="whitespace-pre-wrap break-words text-sm">{step.expected_result ?? "—"}</p>
            {step.note && <p className="mt-1 whitespace-pre-wrap break-words text-xs text-slate-600"><b>Poznámka:</b> {step.note}</p>}
          </div>
          <div>
            <div className="flex gap-1" role="group" aria-label={`Vyhodnocení kroku ${step.step_order}`}>
              {actions.map(action => <button type="button" key={action.value} aria-label={action.label} title={action.label}
                ...
                className={`grid h-8 w-9 place-items-center rounded border text-base font-bold disabled:opacity-40 ${resultColors[action.value]} ${value === action.value ? "border-cyan-600 bg-cyan-50 ring-1 ring-cyan-600" : "border-slate-200 bg-white hover:bg-slate-50"}`}>
                {resultSymbols[action.value]}
              </button>)}
            </div>
            <span className={savingStepId === step.id ? "mt-1 block text-[11px] text-slate-500" : "sr-only"}>{savingStepId === step.id ? "Ukládám…" : actions.find(action => action.value === value)?.label ?? "Nevyhodnoceno"}</span>
            {result?.executed_at && <details className="mt-1 text-[11px] text-slate-500"><summary className="cursor-pointer">Provedení</summary>{executor ?? `Uživatel ${result.executed_by ?? "—"}`}<br />{formatDateTime(result.executed_at)}</details>}
          </div>
        </>}
      </div>;
    })}
