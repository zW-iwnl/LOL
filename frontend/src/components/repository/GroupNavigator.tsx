...
export function GroupNavigator({ index, selection, onSelect, onCreate }: Props) {
  ...
  function renderRow(row: GroupRow) {
    ...
    return <div className={`repository-group-row ${active ? "is-selected" : ""}`} style={{ paddingLeft: Math.min(row.depth * 14, 140) }}
      ...
      <button type="button" className="grid h-8 w-6 shrink-0 place-items-center" disabled={!row.expandable}
        ...
        {row.expandable ? expanded.has(row.key) ? <ChevronDown size={14} /> : <ChevronRight size={14} /> : <span className="text-slate-300">·</span>}
      </button>
      <button data-focus-target type="button" className="min-w-0 flex-1 truncate text-left text-xs" aria-current={active ? "true" : undefined}
        ...
        {row.group.name}{row.group.parent_ids.length > 1 && <span className="ml-1 text-cyan-700">↗ {row.group.parent_ids.length}</span>}
        {!row.includeDescendants && <span className="ml-1 text-amber-700">⊣</span>}
        {query.trim() && !nameMatch && <span className="ml-1 text-slate-500">· tag v obsahu</span>}
      </button>
      <button type="button" className={`grid h-8 w-7 shrink-0 place-items-center ${favorites.includes(row.groupId) ? "text-amber-600" : "text-slate-300 hover:text-amber-600"}`}
  ...
  return <aside className="repository-navigation" aria-label="Struktura skupin">
    <div className="space-y-2 border-b border-slate-200 p-3">
      ...
      <div className="flex justify-between gap-2 text-[11px] text-cyan-800"><button type="button" onClick={() => setExpandedKeys([])}>Sbalit větve</button><button type="button" disabled={!selection} onClick={reveal}>Ukázat vybranou ve struktuře</button></div>
      {searching && <p className="text-[11px] text-slate-500">Každá skupina jednou · ↗ počet rodičů</p>}
    </div>
    ...
    <p className="border-t border-slate-200 p-2 text-xs text-slate-500">{index.ordered.length} skupin · {searching || mode === "list" ? rows.length + " ve výběru" : rows.length + " zobrazených umístění"}</p>
  </aside>;
}
