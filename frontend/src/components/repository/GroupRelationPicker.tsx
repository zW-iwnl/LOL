...
export function GroupRelationPicker({ group, index, suites, busy, error, onClose, onAdd }: {
  ...
  return <AccessibleDialog title="Přidat vazbu" onClose={() => !busy && onClose()}>
    <div className="space-y-3">
      ...
      {loading && <p role="status" className="text-xs">Hledám…</p>}{loadError && <p role="alert" className="text-sm text-rose-700">{loadError}</p>}
      <div className="h-64"><VirtualList items={options} itemKey={item => item.id} label="Dostupné položky" render={item => <button type="button" data-focus-target className={`repository-picker-row ${selected === item.id ? "bg-cyan-50 text-cyan-800" : ""}`} disabled={busy || loading} aria-pressed={selected === item.id} onClick={() => setSelected(item.id)}>#{item.id} {item.name}</button>} /></div>
      ...
      {(kind === "child" || kind === "parent") && <p className="text-xs text-slate-500">Již připojené skupiny a vazby vytvářející cyklus nejsou v nabídce.</p>}
