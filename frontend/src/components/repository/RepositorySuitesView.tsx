...
export function RepositorySuitesView({ suites, groups, tags, selectedSuiteId, onSelect, onCreateSuite, onEditSuite, onDeleteSuite, refreshKey, ...caseActions }: {
  ...
  return <div className="repository-group-workspace">
    ...
    <div className={`repository-panels ${open ? "with-navigation" : ""}`}>
      {open && <aside className="repository-navigation" aria-label="Test suity"><div className="space-y-2 border-b border-border p-3"><h2 className="text-sm font-semibold">Ploché test suity</h2>
      ...
      </div><VirtualList items={filtered} itemKey={suite => suite.id} label="Seznam suit" storageKey="repository-suite-scroll" render={suite => <button data-focus-target type="button" className={`repository-picker-row ${selected?.id === suite.id ? "bg-selected-bg text-link" : ""}`} aria-current={selected?.id === suite.id ? "true" : undefined} title={suite.name} onClick={() => onSelect(suite.id)}><span className="min-w-0 flex-1 truncate">{suite.name}</span><span className="text-muted">{suite.test_case_count}</span></button>} /><p className="border-t p-2 text-xs text-muted">{filtered.length} z {suites.length} suit</p></aside>}
      <section className="repository-detail" aria-label="Detail suity">{selected ? <>
        <header className="space-y-2 border-b p-3"><div className="flex items-start justify-between gap-2"><div><h2 className="text-lg font-semibold">{selected.name}</h2><p className="text-xs text-muted">#{selected.id} · {selected.test_case_count} testů · {selected.is_active ? "Aktivní" : "Neaktivní"}</p></div><ExecutionMenu label="Akce suity"><button type="button" onClick={() => onEditSuite(selected)}>Upravit suitu</button><button type="button" onClick={() => onDeleteSuite(selected)}>Smazat prázdnou suitu</button></ExecutionMenu></div>
          <details className="text-xs"><summary className="cursor-pointer text-muted">Popis a skupiny ({selected.group_ids.length})</summary><p className="mt-1 whitespace-pre-wrap">{selected.description ?? "Bez popisu"}</p><p className="mt-1">{selected.group_ids.map(id => groups.find(group => group.id === id)?.name ?? `#${id}`).join(" · ") || "Bez skupiny"}</p></details>
          {!filtered.some(suite => suite.id === selected.id) && <p className="text-xs text-warning">Vybraná suita je mimo aktuální filtr.</p>}
      ...
      </> : <p className="p-4 text-sm text-muted">Vytvořte první test suitu.</p>}</section>
    </div>
  </div>;
}
