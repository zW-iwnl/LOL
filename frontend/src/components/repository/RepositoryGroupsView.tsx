...
export function RepositoryGroupsView(props: Props) {
  ...
  return <div className="repository-group-workspace">
    ...
    <div className={`repository-panels ${showNavigator ? "with-navigation" : ""}`} style={{ "--repository-navigation-width": `${width}px` } as CSSProperties}>
      ...
      {selection ? <GroupDetail key={selection.groupId} {...props} index={index} selection={selection} onSelect={select} /> : <section className="repository-detail p-4 text-sm">Vyberte skupinu nebo <button type="button" className="text-link underline" onClick={() => setNewOpen(true)}>vytvořte novou</button>.</section>}
    </div>
    {newOpen && <AccessibleDialog title="Nová kořenová skupina" onClose={() => !creating && setNewOpen(false)}><form onSubmit={event => { event.preventDefault(); void create(); }} className="space-y-3">
      ...
      {error && <p role="alert" className="text-sm text-danger">{error}</p>}<button type="submit" className="workspace-button workspace-primary" disabled={creating || !newName.trim()}>Vytvořit skupinu</button>
...
function GroupDetail({ selection, index, onSelect, ...props }: Props & { selection: GroupPlacement; index: ReturnType<typeof createGroupIndex>; onSelect: (placement: GroupPlacement) => void }) {
  ...
  if (!group) return <section className="repository-detail p-4">{state.error ? <p role="alert" className="text-sm text-danger">{state.error}<button className="ml-2 underline" onClick={() => setRefreshKey(value => value + 1)}>Zkusit znovu</button></p> : <p role="status" className="text-sm">Načítám skupinu…</p>}</section>;
  const structure = [
  ...
  return <section className="repository-detail" aria-label="Detail skupiny">
    <header className="space-y-2 border-b border-border p-3">
      <nav aria-label="Cesta skupiny" className="flex flex-wrap gap-1 text-xs text-muted">{selection.path.map((id, i) => <span key={id}>{i > 0 && " › "}<button type="button" onClick={() => onSelect(validatePlacement(index, id, selection.path.slice(0, i + 1)))}>{index.byId.get(id)?.name ?? `#${id}`}</button></span>)}</nav>
      <div className="flex items-start justify-between gap-2"><div><h2 className="text-lg font-semibold">{group.name}</h2><p className="text-xs text-muted">#{group.id} · {group.members.length} přímých suit · {group.test_case_members.length} přímých odkazů · {group.parent_ids.length} rodičů</p></div>
      ...
      {!selection.includeDescendants && <p className="rounded bg-warning-bg p-2 text-xs text-warning">Toto umístění je bez dalších potomků. <button type="button" className="underline" onClick={() => onSelect({ groupId: group.id, path: [group.id], includeDescendants: true })}>Otevřít celou skupinu</button></p>}
    ...
    <div className="repository-detail-scroll p-3">
      {(error || state.error) && <p role="alert" className="mb-2 text-sm text-danger">{error ?? state.error}</p>}{feedback && <p role="status" className="mb-2 text-xs text-success">{feedback}</p>}
      {tab === "content" && <>
        ...
        {contentMode === "direct" && <>
          ...
          <table className="repository-table"><thead><tr><th>Typ</th><th>Název</th><th>Rozsah</th></tr></thead><tbody>{structure.slice(structurePage * 50, structurePage * 50 + 50).map(row => <tr key={`${row.kind}:${row.id}`}><td>{row.kind === "group" ? "Skupina" : "Suita"}</td><td><button type="button" className="text-link" onClick={() => row.kind === "suite" ? props.onOpenSuite(row.id) : onSelect(validatePlacement(index, row.id, [...selection.path, row.id]))}>{row.name}</button></td><td>{row.kind === "group" ? row.include ? "Včetně potomků" : "Bez dalších potomků" : `${props.suites.find(suite => suite.id === row.id)?.test_case_count ?? 0} testů`}</td></tr>)}</tbody></table>
