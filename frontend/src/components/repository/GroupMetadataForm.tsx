...
export function GroupMetadataForm({ group, busy, onSave }: { group: SuiteGroup; busy: boolean; onSave: (name: string, description: string, order: number) => Promise<boolean> }) {
  ...
  return <form onSubmit={event => void submit(event)} className="max-w-2xl space-y-3 text-sm">
    <p className="text-xs text-slate-500">Skupina #{group.id} · změny názvu a popisu se projeví ve všech jejích umístěních.</p>
    {draft && <p className="text-xs text-amber-800">Rozepsané změny jsou uložené v tomto prohlížeči. <button type="button" className="underline" disabled={busy} onClick={() => setDraft(null)}>Zahodit změny</button></p>}
    ...
    {error && <p role="alert" className="text-rose-700">{error}</p>}<button className="workspace-button workspace-primary" type="submit" disabled={busy}>{busy ? "Ukládám…" : "Uložit skupinu"}</button>
    <div className="border-t pt-3"><h3 className="font-medium">Tagy v obsahu skupiny</h3><div className="mt-2 flex flex-wrap gap-1">{group.tags.map(tag => <span key={tag.id} className="rounded bg-slate-100 px-2 py-1 text-xs">{tag.name} ({tag.test_case_count})</span>)}{!group.tags.length && <p className="text-xs text-slate-500">Obsah skupiny zatím nemá tagy.</p>}</div></div>
  </form>;
}
