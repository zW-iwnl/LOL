...
export function CaseDraftEditor({ initial, onSaved, runId, caseAttemptId, onDirtyChange }: { initial: CaseDraft; onSaved?: () => void; runId?: number; caseAttemptId?: number; onDirtyChange?: (dirty: boolean) => void }) {
  ...
  return <div className="space-y-4">
    ...
    {draft.origin_run_id && <Link className="text-sm text-link" to={`/test-runs/${draft.origin_run_id}/execution`}>Zdrojový run #{draft.origin_run_id}</Link>}
    {!editable && <p className="rounded-md bg-warning-bg p-3 text-sm">{draft.status === "submitted" ? "Návrh je uzamčený do rozhodnutí nebo stažení žádosti." : `Návrh upravuje uživatel #${draft.editor_id}.`}</p>}
    {conflict && <div role="alert" className="rounded bg-warning-bg p-3 text-sm">Na serveru je novější revize. Rozepsaný obsah zůstal zachovaný; před dalším uložením zkontrolujte rozdíly.<details className="mt-2"><summary>Aktuální obsah na serveru</summary><div className="mt-2"><CaseContentEditor value={draft.content} onChange={() => undefined} disabled /></div></details><div className="mt-2 flex flex-wrap gap-2"><button type="button" className="workspace-button" disabled={busy} onClick={() => { setConflict(false); setStored({ content, summary, revision: draft.lock_version }); }}>Zkontrolováno, ponechat moje změny</button><button type="button" className="workspace-button" disabled={busy} onClick={() => { setContent(draft.content); setSummary(draft.change_summary); setStored(null); setConflict(false); }}>Použít obsah serveru</button></div></div>}
    <p className="text-xs text-muted">{dirty ? "Rozepsané změny jsou zachované v této kartě prohlížeče; na serveru zatím nejsou uložené." : "Návrh odpovídá uloženému stavu."}</p>
    <CaseContentEditor value={content} onChange={editContent} disabled={!editable || busy} />
    <label className="block text-sm">Důvod změny / zavedení scénáře<textarea disabled={!editable || busy} className="mt-1 w-full rounded-md border border-control px-3 py-2 text-sm bg-surface outline-none focus:border-focus focus:ring-2 focus:ring-focus" maxLength={5000} value={summary} onChange={e => editSummary(e.target.value)} /></label>
    {error && <p role="alert" className="rounded-md bg-danger-bg p-3 text-danger">{error} <button type="button" className="underline" onClick={() => void reloadCurrent()}>Načíst aktuální stav bez ztráty textu</button></p>}
    {message && <p role="status" className="rounded-md bg-success-bg p-3 text-success">{message}</p>}
    <div className="flex flex-wrap gap-3">
      {editable && <><button type="button" disabled={busy || conflict} className="rounded-md border border-border px-4 py-2 disabled:opacity-50 text-sm min-h-11 font-medium transition hover:bg-surface-muted focus-visible:outline-2 focus-visible:outline-offset-2 focus-visible:outline-focus" onClick={() => void action("save")}>Uložit návrh</button>
        <button type="button" disabled={busy || conflict || !summary.trim()} className="rounded-md bg-accent px-4 py-2 text-on-accent disabled:opacity-50 text-sm min-h-11 font-medium transition hover:bg-accent-hover disabled:cursor-not-allowed focus-visible:outline-2 focus-visible:outline-offset-2 focus-visible:outline-focus" onClick={() => void action("submit")}>Odeslat ke schválení</button></>}
      {runId && user?.id === draft.editor_id && draft.status !== "closed" && <button type="button" disabled={busy} className="rounded-md border border-focus px-4 py-2 text-link disabled:opacity-50 text-sm min-h-11 font-medium transition hover:bg-surface-muted focus-visible:outline-2 focus-visible:outline-offset-2 focus-visible:outline-focus" onClick={() => void action("execute")}>{caseAttemptId ? "Provést upravenou verzi" : "Přidat a provést"}</button>}
      {draft.status === "open" && !editable && ["admin", "test_lead"].includes(user?.role ?? "") && <button type="button" disabled={busy} className="rounded-md border border-border px-4 py-2 text-sm min-h-11 font-medium transition hover:bg-surface-muted focus-visible:outline-2 focus-visible:outline-offset-2 focus-visible:outline-focus" onClick={() => void action("takeover")}>Převzít editaci návrhu</button>}
      <Link className="self-center text-link" to={withReturn(reviewId ? `/test-case-approvals/${reviewId}` : `/test-case-approvals?q=${encodeURIComponent(draft.code)}`, runId ? `/test-runs/${runId}/execution?caseAttempt=${caseAttemptId ?? ""}&attempt=${draft.origin_run_attempt_id ?? ""}` : location.pathname + location.search)}>Otevřít schvalování</Link>
    </div>
  </div>;
}
