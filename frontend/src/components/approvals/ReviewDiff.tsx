...
export function ReviewDiff({ changes, firstVersion, onComment }: { changes: ReviewDetail["changes"]; firstVersion: boolean; onComment?: (field: string) => void }) {
  return <div className="space-y-2">{firstVersion && <p className="text-xs text-info">Nový scénář – první publikovaná verze.</p>}{!changes.length && <p className="text-sm text-muted">Obsah se neliší.</p>}{changes.map(change => {
    ...
    return <section key={change.field} className="rounded border border-border" id={`change-${change.field}`}><header className="flex items-center justify-between gap-2 bg-surface-muted px-3 py-2 text-xs"><h3 className="font-semibold">{changeLabel(change.field, before, after)} · {kind}</h3>{onComment && <button type="button" className="text-link" onClick={() => onComment(change.field)}>Připomínkovat</button>}</header>
      <div className="grid grid-cols-2 gap-2 px-3 pt-2 text-[11px] text-muted"><span>Před změnou</span><span>Navržená verze</span></div>
      {fields.map(field => <div key={field} className="px-3 py-2">{step && <p className="mb-1 text-[11px] text-muted">{fieldLabels[field]}</p>}<div className="grid grid-cols-2 gap-2 text-xs"><p className="min-w-0 whitespace-pre-wrap break-words rounded bg-danger-bg p-2">{display(step ? before?.[field] : change.before)}</p><p className="min-w-0 whitespace-pre-wrap break-words rounded bg-success-bg p-2">{display(step ? after?.[field] : change.after)}</p></div></div>)}
    </section>;
  })}</div>;
}
