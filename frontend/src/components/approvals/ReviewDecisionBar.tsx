...
export function ReviewDecisionBar({ review, busy, reason, error, onReason, onDecide, onClaim, onBlocking }: {
  ...
  if (review.status !== "pending") return <footer className="workspace-decision-bar text-xs text-slate-500">Žádost je uzavřená. {review.decision_reason}</footer>;
  return <footer className="workspace-decision-bar space-y-2">
    {caps?.approval_reason && <p className="text-xs text-amber-800">{caps.approval_reason} {review.comments.some(c => c.is_blocking && !c.resolved_at) && <button type="button" className="underline" onClick={onBlocking}>Otevřít připomínky</button>}</p>}
