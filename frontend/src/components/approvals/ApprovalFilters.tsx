...
export function ApprovalFilters({ params, tab, onChange }: { params: URLSearchParams; tab: string; onChange: (key: string, values: string[]) => void }) {
  ...
  return <div className="space-y-2"><div className="flex flex-wrap items-center gap-2"><input aria-label="Kód nebo název" type="search" maxLength={200} className="workspace-input min-w-40 flex-1" placeholder="Kód nebo název scénáře…" value={params.get("q") ?? ""} onChange={e => onChange("q", [e.target.value])} />
    ...
    {!!active.length && <div className="flex flex-wrap gap-1">{active.map(([key, value], i) => <button key={`${key}:${value}:${i}`} type="button" className="rounded bg-selected-bg px-2 py-1 text-[11px] text-link" onClick={() => onChange(key, params.getAll(key).filter(item => item !== value))}>{label(key, value)} ×</button>)}</div>}
    {open && <div className="grid gap-2 rounded border border-border bg-surface p-2 md:grid-cols-3 xl:grid-cols-4">{options.loading && <p role="status" className="text-xs">Načítám filtry…</p>}{options.error && <p role="alert" className="text-xs text-danger">{options.error}</p>}
