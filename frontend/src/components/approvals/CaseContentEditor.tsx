...
const input = "mt-1 w-full rounded-md border border-control bg-surface px-3 py-2 text-sm outline-none transition focus:border-focus focus:ring-2 focus:ring-focus disabled:bg-surface-muted";
export function CaseContentEditor({ value, onChange, disabled = false }: { value: DraftContent; onChange: (v: DraftContent) => void; disabled?: boolean }) {
  ...
  return <fieldset disabled={disabled} className="space-y-4 disabled:opacity-70">
    ...
    {tags.error && <p role="alert" className="text-danger">{tags.error}</p>}
    ...
    {value.steps.map((step, index) => <section className="space-y-3 rounded-md border border-border bg-surface-muted p-4" key={step.step_key}>
      <div className="flex flex-wrap items-center gap-3"><strong>Krok {index + 1}</strong>
        <select aria-label={`Typ kroku ${index + 1}`} className="rounded-md border border-control px-3 py-2 text-sm bg-surface outline-none focus:border-focus focus:ring-2 focus:ring-focus" value={step.step_type} onChange={e => stepChange(index, { step_type: e.target.value as DraftStep["step_type"] })}><option value="test">Testovací</option><option value="information">Informační</option></select>
        <button type="button" disabled={index === 0} onClick={() => move(index, -1)} className="min-h-11 rounded-md border border-border bg-surface px-3 py-2 text-sm font-medium hover:bg-surface-muted disabled:opacity-40">↑ Nahoru</button>
        <button type="button" disabled={index === value.steps.length - 1} onClick={() => move(index, 1)} className="min-h-11 rounded-md border border-border bg-surface px-3 py-2 text-sm font-medium hover:bg-surface-muted disabled:opacity-40">↓ Dolů</button>
        <button type="button" className="ml-auto text-danger" onClick={() => set("steps", value.steps.filter((_, i) => i !== index).map((s, i) => ({ ...s, step_order: i + 1 })))}>Odebrat krok</button>
      </div>
    ...
    <button type="button" className="rounded-md border border-focus px-3 py-2 text-link text-sm min-h-11 font-medium transition hover:bg-surface-muted focus-visible:outline-2 focus-visible:outline-offset-2 focus-visible:outline-focus" onClick={() => set("steps", [...value.steps, { step_key: crypto.randomUUID(), step_order: value.steps.length + 1, step_type: "test", action: "", expected_result: null, note: null, test_data: null }])}>Přidat krok</button>
    <p className="text-xs text-muted">Do scénáře nevkládejte skutečná hesla ani tokeny. Používejte odkazy na testovací data.</p>
  </fieldset>;
}
