...
export function RunVersionSelector({ caseId, attemptId, currentVersionId, onClose, onExecuted }: {
  ...
  return <AccessibleDialog title="Nový pokus nad schválenou verzí" onClose={onClose} panelClassName="max-w-4xl"><div className="space-y-4">
    ...
    <select aria-label="Schválená verze" className="w-full rounded-md border border-control px-3 py-2 text-sm bg-surface outline-none focus:border-focus focus:ring-2 focus:ring-focus" value={selected?.id ?? ""} onChange={e => setSelected(versions.data?.items.find(v => v.id === Number(e.target.value)) ?? null)}><option value="">Vyberte verzi</option>{versions.data?.items.filter(v => ["approved", "legacy_import"].includes(v.approval_state)).map(v => <option key={v.id} value={v.id}>Verze {v.version_number} · {v.content_snapshot.title}</option>)}</select>
    {(versions.error || changes.error || error) && <p role="alert" className="text-danger">{versions.error || changes.error || error}</p>}
    {selected && <><CaseSnapshotView content={selected.content_snapshot} /><details><summary>Porovnání s prováděnou verzí</summary>{currentVersionId ? changes.data?.map(c => <div className="my-2 rounded-md border border-border px-3 py-2 text-sm bg-surface outline-none focus:border-focus focus:ring-2 focus:ring-focus" key={c.field}><strong>{c.field}</strong><pre className="whitespace-pre-wrap break-words text-xs">{JSON.stringify({ před: c.before, po: c.after }, null, 2)}</pre></div>) : <p>Legacy provedení nemá doložený odkaz na obsahovou verzi; porovnejte s jeho historickým snapshotem.</p>}</details></>}
    <button disabled={!selected || busy || changes.loading || Boolean(changes.error)} className="rounded-md bg-accent px-4 py-2 text-on-accent disabled:opacity-50 text-sm min-h-11 font-medium transition hover:bg-accent-hover disabled:cursor-not-allowed focus-visible:outline-2 focus-visible:outline-offset-2 focus-visible:outline-focus" onClick={() => void execute()}>Potvrdit verzi a založit nový pokus</button>
