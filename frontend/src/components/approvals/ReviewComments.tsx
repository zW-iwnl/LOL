...
export function ReviewComments({ review, body, field, blocking, busy, onBody, onField, onBlocking, onAdd, onResolve, onChange }: {
  ...
  return <div className="space-y-3"><label className="flex items-center gap-2 text-xs"><input type="checkbox" checked={openOnly} onChange={e => setOpenOnly(e.target.checked)} />Jen otevřené připomínky</label>
    {review.comments.filter(c => !openOnly || !c.resolved_at).map(c => <article key={c.id} className={`rounded border p-3 text-sm ${c.is_blocking && !c.resolved_at ? "border-amber-200 bg-amber-50" : "border-slate-200"}`}><div className="flex flex-wrap justify-between gap-1 text-xs text-slate-500"><span>{c.author_name ?? `Uživatel #${c.author_id}`} · {c.is_blocking ? "Blokující" : "Poznámka"} · {c.resolved_at ? "Vyřešeno" : "Otevřeno"}</span>{c.field_path && <button type="button" className="text-cyan-800" onClick={() => onChange(c.field_path!)}>Zobrazit změnu</button>}</div><p className="my-2 whitespace-pre-wrap break-words">{c.body}</p>{caps?.resolvable_comment_ids.includes(c.id) && <button type="button" className="workspace-button" disabled={busy} onClick={() => onResolve(c.id)}>Označit jako vyřešené</button>}</article>)}
    {!review.comments.some(c => !openOnly || !c.resolved_at) && <p className="text-sm text-slate-500">Žádné {openOnly ? "otevřené " : ""}připomínky.</p>}
